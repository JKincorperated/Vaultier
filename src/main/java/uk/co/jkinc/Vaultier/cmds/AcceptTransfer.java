...
public class AcceptTransfer implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        ...
        if (Vaultier.econ.getBalance(transaction.Payer) < transaction.value) {
            sender.sendMessage(ChatColor.RED + "You do not have enough to afford this...");
            return true;
        }


        sender.sendMessage(ChatColor.GREEN + "Transaction accepted.");

        transaction.state = Transaction.Status.SUCCESS;
        Vaultier.database.db.Transactions.put(transactionID, transaction);

        Vaultier.econ.withdrawPlayer(transaction.Payer, transaction.value);
        Vaultier.econ.depositPlayer(transaction.Payee, transaction.value);
